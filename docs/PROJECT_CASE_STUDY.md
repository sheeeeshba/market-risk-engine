...
Translate a transparent multi-asset portfolio into explainable P&L, current VaR/ES, rolling forecasts, exception diagnostics, additive risk drivers, deterministic stresses, and a management-style report.
...
The implementation separates data alignment, portfolio accounting, risk models, backtesting, contributions, stresses, and reporting behind testable interfaces. Every forecast uses only information available through its forecast date. Random seeds, configuration and factor hashes, runtime versions, and outputs are recorded.
...
Automated tests cover signs, units, data gaps, accounting, exact tail weights, covariance and seed behaviour, backtest edge cases, scenario reconciliation, and an end-to-end small run. The verification manifest identifies commands, hashes, timestamps, input snapshot, exit status, and verified artifacts.
