...
The bullets below are safe to use now because they describe verified engineering scope, not historical market performance or business impact.
...
Designed and validated an educational multi-asset market-risk engine that converts portfolio positions into hypothetical P&L, VaR and Expected Shortfall, rolling exception diagnostics, additive risk contributions, hypothetical stresses, and a reproducible management report. The public repository includes a deterministic synthetic snapshot for credential-free review; its values are clearly labeled as implementation evidence rather than historical performance.
...
Do not quote market-loss values, historical-crisis performance, empirical model superiority, or business impact until a permitted real-data snapshot has been independently verified and the corresponding evidence manifest has been regenerated.
